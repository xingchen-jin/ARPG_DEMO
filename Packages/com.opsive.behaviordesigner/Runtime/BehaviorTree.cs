...
[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("Opsive.BehaviorDesigner.Runtime.Systems")]
namespace Opsive.BehaviorDesigner.Runtime
{
    ...
    public class BehaviorTree : MonoBehaviour, IGraph, IGraphComponent, ISharedVariableContainer
    {
        ...
        private void InitializeBranch(World world, Entity entity, ECSVariableRegistry registry, IEventNode eventTask)
        {
            ...
            BehaviorTreeExecution.AddCleanupSystems(world, behaviorTreeSystemGroup);
            ...
            for (int i = eventTask.ConnectedIndex; i < m_Data.LogicNodes.Length; ++i) {
                ...
                if (m_Data.LogicNodes[i] is IAuthoringTask authoringTask) {
                    ...
                    BehaviorTreeExecution.AddTaskObjectSystem(world, traversalTaskSystemGroup);
                ...
                if (m_Data.LogicNodes[i] is IConditional && m_Data.LogicNodes[i].ParentIndex != ushort.MaxValue) {
                    ...
                    if (m_Data.LogicNodes[i] is IAuthoringTask conditionalAuthoringTask) {
                        ...
                        reevaluateSystem = BehaviorTreeExecution.TaskObjectReevaluateSystemType;
                    }
            ...
            if (canReevaluate) {
                BehaviorTreeExecution.AddReevaluateSystem(world, world.GetOrCreateSystemManaged<BeforeTraversalSystemGroup>());
            }
        ...
        /// <summary>
        /// Returns the status for the node with the specified runtime index.
        /// </summary>
        /// <param name="runtimeIndex">The runtime index of the node.</param>
        /// <returns>The task status.</returns>
        public TaskStatus GetRuntimeNodeStatus(int runtimeIndex)
        {
            if (!Application.isPlaying || m_Entity == Entity.Null || m_World == null || !m_World.IsCreated || !m_World.EntityManager.Exists(m_Entity) ||
                !m_World.EntityManager.HasBuffer<TaskComponent>(m_Entity)) {
                return TaskStatus.Inactive;
            }

            var taskComponents = m_World.EntityManager.GetBuffer<TaskComponent>(m_Entity);
            if (runtimeIndex < 0 || runtimeIndex >= taskComponents.Length) {
                return TaskStatus.Inactive;
            }

            var taskComponent = taskComponents[runtimeIndex];
