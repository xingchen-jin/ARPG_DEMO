...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Composites
{
    ...
    public class PrioritySelector : ECSCompositeTask<PrioritySelectorTaskSystem, PrioritySelectorComponent, PrioritySelectorFlag>, IParentNode, ISavableTask, ICloneable
    {
        ...
        public override int AddBufferElement(World world, Entity entity, ECSVariableRegistry registry, GameObject gameObject)
        {
            ...
            if (!world.EntityManager.HasBuffer<PrioritySelectorSortedOrderComponent>(entity)) {
                world.EntityManager.AddBuffer<PrioritySelectorSortedOrderComponent>(entity);
            }
    ...
    public struct PrioritySelectorComponent : IBufferElementData
    {
        ...
        [Tooltip("The start index within the PrioritySelectorSortedOrderComponent buffer.")]
        public int SortedOrderStartIndex;
        [Tooltip("The number of sorted order entries used by this priority selector.")]
        public ushort SortedOrderLength;
    }
    ...
    public partial struct PrioritySelectorTaskSystem : ISystem
    {
        ...
        private void OnCreate(ref SystemState state)
        {
            m_WithValuesQuery = SystemAPI.QueryBuilder()
                ...
                .WithAllRW<PrioritySelectorSortedOrderComponent>()
        ...
        private partial struct PrioritySelectorWithValuesJob : IJobEntity
        {
            ...
            /// <param name="prioritySelectorSortedOrderComponents">An array of PrioritySelectorSortedOrderComponents.</param>
            ...
            public void Execute(ref DynamicBuffer<BranchComponent> branchComponents, ref DynamicBuffer<TaskComponent> taskComponents,
                ref DynamicBuffer<PrioritySelectorComponent> prioritySelectorComponents, ref DynamicBuffer<PrioritySelectorSortedOrderComponent> prioritySelectorSortedOrderComponents,
                ref DynamicBuffer<PriorityValueComponent> priorityValueComponents)
            {
                for (int i = 0; i < prioritySelectorComponents.Length; ++i) {
                    ...
                    if (taskStatus == TaskStatus.Queued) {
                        ...
                        if (prioritySelectorComponent.SortedOrderStartIndex < 0 || prioritySelectorComponent.SortedOrderLength != orderCount ||
                            prioritySelectorComponent.SortedOrderStartIndex + prioritySelectorComponent.SortedOrderLength > prioritySelectorSortedOrderComponents.Length) {
                            prioritySelectorComponent.SortedOrderStartIndex = prioritySelectorSortedOrderComponents.Length;
                            prioritySelectorComponent.SortedOrderLength = (ushort)orderCount;
                            for (int j = 0; j < orderCount; ++j) {
                                prioritySelectorSortedOrderComponents.Add(new PrioritySelectorSortedOrderComponent());
                            }
                        }

                        for (ushort j = 0; j < orderCount; ++j) {
                            prioritySelectorSortedOrderComponents[prioritySelectorComponent.SortedOrderStartIndex + j] = new PrioritySelectorSortedOrderComponent() { Index = j };
                        }

                        for (int a = 0; a < orderCount; ++a) {
                            for (int b = a + 1; b < orderCount; ++b) {
                                var sortedOrderIndexA = prioritySelectorComponent.SortedOrderStartIndex + a;
                                var sortedOrderIndexB = prioritySelectorComponent.SortedOrderStartIndex + b;
                                ref var entryA = ref priorityItems[prioritySelectorSortedOrderComponents[sortedOrderIndexA].Index];
                                ref var entryB = ref priorityItems[prioritySelectorSortedOrderComponents[sortedOrderIndexB].Index];
                                ...
                                if (valueB > valueA) {
                                    var t = prioritySelectorSortedOrderComponents[sortedOrderIndexA];
                                    prioritySelectorSortedOrderComponents[sortedOrderIndexA] = prioritySelectorSortedOrderComponents[sortedOrderIndexB];
                                    prioritySelectorSortedOrderComponents[sortedOrderIndexB] = t;
                                }
                            }
                        }

                        for (int j = 0; j < orderCount; ++j) {
                            var sortedOrderIndex = prioritySelectorComponent.SortedOrderStartIndex + j;
                            var relIdx = prioritySelectorSortedOrderComponents[sortedOrderIndex].Index;
                            prioritySelectorSortedOrderComponents[sortedOrderIndex] = new PrioritySelectorSortedOrderComponent() { Index = priorityItems[relIdx].TaskIndex };
                        }

                        prioritySelectorComponent.ActiveRelativeChildIndex = 0;
                        prioritySelectorComponents[i] = prioritySelectorComponent;

                        branchComponent.NextIndex = prioritySelectorSortedOrderComponents[prioritySelectorComponent.SortedOrderStartIndex + prioritySelectorComponent.ActiveRelativeChildIndex].Index;
                        branchComponents[taskComponent.BranchIndex] = branchComponent;
                    ...
                    var childTaskComponent = taskComponents[prioritySelectorSortedOrderComponents[prioritySelectorComponent.SortedOrderStartIndex + prioritySelectorComponent.ActiveRelativeChildIndex].Index];
                    ...
                    if (prioritySelectorComponent.ActiveRelativeChildIndex == prioritySelectorComponent.SortedOrderLength - 1 ||
                        ...
                        var nextIndex = prioritySelectorSortedOrderComponents[prioritySelectorComponent.SortedOrderStartIndex + prioritySelectorComponent.ActiveRelativeChildIndex].Index;
